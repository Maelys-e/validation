...
def program_attente_state(states) :
    def action_piece_1(config) :
        config.PC_alice = w
        config.PC_bob = i
        config.state = (config.PC_alice, config.PC_bob)
    ...
    def action_piece_5(config) :
        ...
        config.PC_bob = c
        config.state = (config.PC_alice, config.PC_bob)
    p5 = algo.Piece("p5", lambda conf : (conf.PC_alice == i and conf.PC_bob == w) or (conf.PC_alice == i and conf.PC_bob == r), action_piece_5)
    def action_piece_6(config) :
        config.PC_alice = c
        config.PC_bob = i
        config.state = (config.PC_alice, config.PC_bob)
    ...
    def action_piece_7(config) :
        ...
        config.PC_bob = c
        config.state = (config.PC_alice, config.PC_bob)
    p7 = algo.Piece("p7", lambda conf : (conf.PC_alice == i and conf.PC_bob == c) or (conf.PC_alice == w and conf.PC_bob == r), action_piece_7)
    def action_piece_8(config) :
        config.PC_alice = c
        config.PC_bob = w
        config.state = (config.PC_alice, config.PC_bob)
    p8 = algo.Piece("p8", lambda conf : conf.PC_alice == c and conf.PC_bob == i, action_piece_8)
    def action_piece_9(config) :
        config.PC_alice = i
        config.PC_bob = r
        config.state = (config.PC_alice, config.PC_bob)
    p9 = algo.Piece("p9", lambda conf : conf.PC_alice == c and conf.PC_bob == r, action_piece_9)
    def action_piece_10(config) :
        config.PC_alice = w
        config.PC_bob = r
        config.state = (config.PC_alice, config.PC_bob)
    p10 = algo.Piece("p10", lambda conf : (conf.PC_alice == w and conf.PC_bob == w) or (conf.PC_alice == i and conf.PC_bob == r), action_piece_10)
    def action_piece_11(config) :
        config.PC_alice = c
        config.PC_bob = r
        config.state = (config.PC_alice, config.PC_bob)
    p11 = algo.Piece("p11", lambda conf : conf.PC_alice == w and conf.PC_bob == r, action_piece_11)
    roots = []
    for state in states :
        roots.append(AliceBobConfig(state.PC_alice, state.PC_bob))
    soup = algo.Soup(roots, [p1, p2, p3, p4 ,p5, p6, p7, p8, p9, p10, p11])
    return soup

def program_attente() :
    return program_attente_state([AliceBobConfig()])
